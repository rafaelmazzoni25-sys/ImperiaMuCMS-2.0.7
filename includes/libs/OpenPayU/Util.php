...
class OpenPayU_Util
{
    public static function generateSignData($data, $algorithm = "SHA-256", $merchantPosId = "", $signatureKey = "")
    {
    ...
    public static function verifySignature($message, $signature, $signatureKey, $algorithm = "MD5")
    {
        ...
        if (isset($signature)) {
            if ($algorithm == "MD5") {
                ...
                if (in_array($algorithm, ["SHA", "SHA1", "SHA-1"])) {
                    $hash = sha1($message . $signatureKey);
                } else {
                    if (in_array($algorithm, ["SHA-256", "SHA256", "SHA_256"])) {
    ...
    public static function buildJsonFromArray($data, $rootElement = "")
    {
        ...
        $data = $this::setSenderProperty($data);
    ...
    public static function parseArrayToObject($array)
    {
        ...
        if (is_array($array) && 0 < count($array)) {
            foreach ($array as $name => $value) {
                ...
                if (isset($name)) {
                    if (is_numeric($name)) {
                        $object[] = $this::parseArrayToObject($value);
                    } else {
    ...
    public static function convertArrayToHtmlForm($array, $namespace = "", &$outputFields)
    {
        ...
        foreach ($array as $key => $value) {
            ...
            if (is_array($value)) {
                $htmlOutput .= $this::convertArrayToHtmlForm($value, $key, $outputFields);
            } else {
    ...
    {
        $data["properties"][0] = ["name" => "sender", "value" => OpenPayU_Configuration::getFullSenderName()];
        return $data;
    }
