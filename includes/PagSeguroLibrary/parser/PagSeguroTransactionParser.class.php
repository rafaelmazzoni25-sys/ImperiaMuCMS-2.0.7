        ...
        if (isset($data["transactions"]) && is_array($data["transactions"])) {
            ...
            if (isset($data["transactions"]["transaction"][0])) {
                ...
                foreach ($data["transactions"]["transaction"] as $key => $value) {
                    $transactions[$i++] = $this::parseTransactionSummary($value);
                }
            } else {
                $transactions[0] = $this::parseTransactionSummary($data["transactions"]["transaction"]);
            }
        ...
        if (isset($data["items"]["item"]) && is_array($data["items"]["item"])) {
            ...
            if (isset($data["items"]["item"][0])) {
                foreach ($data["items"]["item"] as $key => $value) {
                    $item = $this::parseTransactionItem($value);
                    $items[$i] = $item;
                    $i++;
                }
            } else {
                $items[0] = $this::parseTransactionItem($data["items"]["item"]);
            }
