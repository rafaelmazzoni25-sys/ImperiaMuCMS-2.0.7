...
$voteIp = isset($_POST["ip_addr"]) ? $_POST["ip_addr"] : NULL;
$credits = $dB->query_fetch_single("SELECT votesite_reward, votesite_id FROM IMPERIAMUCMS_VOTE_SITES WHERE postback_type = 'mmtop200'");
...
if (!in_array($_SERVER["REMOTE_ADDR"], ["70.32.23.49"])) {
    $error = true;
    $code = 102;
}
    ...
    try {
        ...
        switch ($configSettings["config_user_col_id"]) {
            ...
            case "email":
                ...
                $_GET["page"] = "api";
                $_GET["subpage"] = "mmtop200";
                $creditSystem->addCredits($credits);
                $add_logs_data = [$user_id, $voteIp, $votesite_id, time(), 1, "mmtop200"];
                ...
                $common->accountLogs($userId, "vote", "Received reward for voting on mmTop200.", $logDate);
                $result = true;
                echo "OK";
                break;
            default:
