...
$Vote = new Vote();
$userId = isset($_POST["pingUsername"]) ? $_POST["pingUsername"] : NULL;
$voteIp = isset($_POST["VoterIP"]) ? $_POST["VoterIP"] : NULL;
$success = isset($_POST["Successful"]) ? $_POST["Successful"] : NULL;
$reason = isset($_POST["Reason"]) ? $_POST["Reason"] : NULL;
$credits = $dB->query_fetch_single("SELECT votesite_reward, votesite_id FROM IMPERIAMUCMS_VOTE_SITES WHERE postback_type = 'gtop100'");
$votesite_id = $credits["votesite_id"];
$credits = $credits["votesite_reward"];
$delay = mconfig("delay");
$result = false;
$error = false;
if (!check_value($userId) || !check_value($voteIp) || !check_value($success)) {
    $error = true;
    $code = 100;
}
...
if (!in_array($_SERVER["REMOTE_ADDR"], ["104.28.15.89", "173.245.58.198", "173.245.59.206", "198.148.82.98", "2610:150:8019::1:fc02"])) {
    $error = true;
    $code = 102;
}
    ...
    try {
        ...
        switch ($configSettings["config_user_col_id"]) {
            ...
            case "email":
                ...
                $_GET["page"] = "api";
                $_GET["subpage"] = "gtop100";
                $creditSystem->addCredits($credits);
                $add_logs_data = [$user_id, $voteIp, $votesite_id, time(), 1, "gtop100"];
                ...
                $common->accountLogs($userId, "vote", "Received reward for voting on GTop100.", $logDate);
                $result = true;
                echo "OK";
                break;
            default:
