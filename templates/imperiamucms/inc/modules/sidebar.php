...
if ((config('SQL_USE_2_DB', true) == true)) {
    $activePlayers = $dB2->query_fetch_single("
        SELECT COUNT(memb___id) as totalActive 
        FROM MEMB_STAT WHERE ConnectTM > ?",
        array($yesterday));
} else {
    $activePlayers = $dB->query_fetch_single("
        SELECT COUNT(memb___id) as totalActive 
        FROM MEMB_STAT WHERE ConnectTM > ?",
        array($yesterday));
}
    ...
    <div class="panel-body">
        <script type="text/javascript">
            function loadServerStatus() {
                $(\'#serverstatus\').load(\'' . __BASE_URL__ . 'templates/imperiamucms/ajax/server_online.php\', function() {
        ...
        <ul class="sidebar-list panel-listing">
            ...
            <li>' . lang('res_template_txt_42', true) . ' <span class="pull-right">' . $activePlayers['totalActive'] . '</span></li>
        </ul>
    </div>
</div>';
...
<div class="panel panel-default server-info-home-panel">
    ...
    <div class="panel-body">
        <ul class="sidebar-list panel-listing">
            <li>' . lang('template_txt_31', true) . ' <span class="pull-right">Season 14</span></li>
                            ...
                            foreach ($latestItems as $item) {
                                ...
                                <tr>
                                    <td align="left" onmouseover="Tip(' . $Items->generateItemTooltip($itemInfo, 1, 1, 1, 1, 0, 0) . ')" onmouseout="UnTip()" <img src="' . $itemInfo['thumb'] . '" class="m items-inventory-item-bg">
                                        <span style="cursor:pointer;color:' . $itemInfo['color'] . ';background-color:' . $itemInfo['anco'] . '">' . $itemInfo['name'] . '</span>
                                    </td>
                                    <td align="right" class="items-inventory-item-bg">' . $Market->showPrice($item['price_type'], $item['price']) . '</td>
                                </tr>';
                            }
                            echo '</table>';
                        }
                ...
                foreach ($auctions as $thisAuction) {
                    ...
                    if (is_array($items)) {
                        ...
                        foreach ($items as $thisItem) {
                            ...
                            if ($totalWidth <= 6) {
                                $itemsTxt .= '
                            <div class="auction-widget-item" onmouseover="Tip(' . $Items->generateItemTooltip($itemInfo, 1, 1, 1, 1, 0, 0) . ')" onmouseout="UnTip()">
                                <img src="' . $itemInfo['thumb'] . '" height="' . $height . 'px" style="top: ' . $padding . 'px">
                            </div>';
                            } else {
                                $moreItems++;
                            }
