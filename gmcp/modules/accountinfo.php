...
if (check_value($_GET["id"])) {
    try {
        ...
        echo "</tr><tr><td>WCoinP:</td>";
        echo "<td><b>" . number_format($balance2["WCoinP"]) . "</b></td>";
        echo "</tr><tr><td>GoblinPoint:</td>";
        ...
        if ($accountInfoConfig["showCharacters"]) {
            ...
            if (is_array($accountCharacters)) {
                ...
                foreach ($accountCharacters as $characterName) {
                    echo "<tr>";
                    echo "<td><a href=\"" . gmcp_base("editcharacter&name=" . hex_encode($characterName)) . "\">" . $common->replaceHtmlSymbols($characterName) . "</a></td>";
                    echo "</tr>";
                }
                echo "</table>";
            } else {
        ...
        echo "<div class=\"panel panel-primary\"><div class=\"panel-heading\">Web Bank Information</div><div class=\"panel-body\"><table class=\"table table-no-border table-hover\"><tr><td width=\"25%\">Zen:</td>";
        ...
        if (is_array($customitems)) {
            foreach ($customitems as $thisItem) {
                $customItemAmount = $dB->query_fetch_single("SELECT " . $thisItem["name"] . " FROM IMPERIAMUCMS_WEB_BANK WHERE AccountID = ?", [$_SESSION["username"]]);
                if ($i % 2 == 0) {
                    echo "<tr>";
                }
                echo "\r\n                    <td>" . $thisItem["name"] . "</td>\r\n                    <td><b>" . number_format($customItemAmount[$thisItem["name"]]) . "</b></td>";
                if ($i % 2 == 0) {
                    echo "</tr>";
                }
                $i++;
        ...
        echo "</table></div></div></div></div><div class=\"row\"><div class=\"col-md-12\">";
        ...
        echo "<div class=\"panel panel-primary\"><div class=\"panel-heading\">Items Inventory Information</div><div class=\"panel-body\"><table class=\"table table-no-border table-hover\"><tr><th>#</th><th>Item</th><th>Price</th><th>Type</th><th>Date</th></tr>";
        $Market = new Market();
        $Items = new Items();
