...
if (check_value($_GET["id"]) && is_numeric($_GET["id"])) {
    ...
    if (check_value($_POST["submit"])) {
        if (check_value($_POST["title"]) && check_value($_POST["pos"])) {
            if (is_numeric($_POST["pos"])) {
                ...
                $insert = $dB->query("UPDATE IMPERIAMUCMS_CASHSHOP_CATEGORIES SET title = ?, position = ? WHERE id = ?", [$title, $pos, $id]);
                if ($insert) {
    ...
    $catData = $dB->query_fetch_single("SELECT * FROM IMPERIAMUCMS_CASHSHOP_CATEGORIES WHERE id = ?", [$id]);
