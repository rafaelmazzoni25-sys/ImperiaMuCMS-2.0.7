...
echo "<h1 class=\"page-header\">Add New Package</h1>\r\n\r\n";
$Webshop = new Webshop();
if (check_value($_POST["add_package"])) {
    $Webshop->addPackage($_POST["setting_1"], $_POST["setting_2"], $_POST["setting_3"], $_POST["setting_4"], $_POST["setting_5"], $_POST["setting_6"], $_POST["setting_7"], $_POST["req_class"]);
    $package_id = $dB->query_fetch_single("SELECT TOP 1 id FROM IMPERIAMUCMS_WEBSHOP_PACKAGES WHERE name=? AND price=? AND payment_type=? AND on_sale=? AND image=?", [$_POST["setting_1"], $_POST["setting_2"], $_POST["setting_3"], $_POST["setting_4"], $_POST["setting_5"]]);
    $package_id = $package_id["id"];
    $i = 0;
    while ($i < 50) {
        $index = "item" . $i;
        if (!($_POST[$index] == NULL || $_POST[$index] == __ITEM_EMPTY__)) {
            $Webshop->addPackageItem($package_id, $_POST[$index]);
        }
        $i++;
    }
}
echo "<a class=\"btn btn-primary\" href=\"" . admincp_base("webshop_packages") . "\">PACKAGES MANAGER</a><br/><br/>";
