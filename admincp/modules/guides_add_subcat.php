...
echo "<h1 class=\"page-header\">Create New Subcategory for Guides</h1>\r\n\r\n";
if (check_value($_POST["submit"])) {
    if (check_value($_POST["title"]) && check_value($_POST["cat"]) && check_value($_POST["pos"])) {
        if (is_numeric($_POST["pos"])) {
            ...
            $insert = $dB->query("INSERT INTO IMPERIAMUCMS_GUIDES_SUBCATEGORIES (title, category_id, position, active) VALUES (?, ?, ?, ?)", [$title, $cat, $pos, 1]);
            if ($insert) {
