...
if (check_value($_GET["id"]) && is_numeric($_GET["id"])) {
    ...
    if (check_value($_POST["submit"])) {
        if (check_value($_POST["title"]) && check_value($_POST["cat"]) && check_value($_POST["pos"])) {
            if (is_numeric($_POST["pos"])) {
                ...
                $insert = $dB->query("UPDATE IMPERIAMUCMS_GUIDES_SUBCATEGORIES SET category_id = ?, title = ?, position = ? WHERE id = ?", [$cat, $title, $pos, $id]);
                if ($insert) {
    ...
    $subcatData = $dB->query_fetch_single("SELECT * FROM IMPERIAMUCMS_GUIDES_SUBCATEGORIES WHERE id = ?", [$id]);
