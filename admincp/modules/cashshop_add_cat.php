...
echo "<h1 class=\"page-header\">Create New Category for Cash Shop</h1>\r\n\r\n";
if (check_value($_POST["submit"])) {
    if (check_value($_POST["title"]) && check_value($_POST["pos"])) {
        if (is_numeric($_POST["pos"])) {
            $title = addslashes($_POST["title"]);
            $pos = xss_clean($_POST["pos"]);
            $insert = $dB->query("INSERT INTO IMPERIAMUCMS_CASHSHOP_CATEGORIES (title, position, active) VALUES (?, ?, ?)", [$title, $pos, 1]);
            if ($insert) {
