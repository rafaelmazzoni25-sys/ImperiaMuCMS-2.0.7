...
if (!isLoggedIn()) {
    ...
    if (defined(__RESPONSIVE__) && __RESPONSIVE__ == "TRUE") {
        ...
        echo "\r\n    <h3>\r\n        " . lang("itemsinv_txt_1", true) . "\r\n        " . $breadcrumb . "\r\n    </h3>";
        if (mconfig("active")) {
            echo "\r\n    <div class=\"row desc-row\">\r\n        <div class=\"col-xs-12\">\r\n            " . lang("itemsinv_txt_4", true) . "\r\n        </div>\r\n    </div>";
            $General = new xGeneral();
            $General->ifn9fJgdGKPP_check_jhd7cBDv_Module_fnub7Hda_License("items");
            $General->fjbaYbddafFF_check_jf7bSC_Local_kgfjJG_Module_jGGrOZnf_License("items");
            ...
            echo "\r\n    <div class=\"table-responsive rankings-table\">\r\n        <table class=\"table table-hover text-center\">\r\n            <tr>\r\n                <th class=\"headerRow\">#</th>\r\n                <th class=\"headerRow\">" . lang("itemsinv_txt_5", true) . "</th>\r\n                <th class=\"headerRow\">" . lang("itemsinv_txt_6", true) . "</th>\r\n                <th class=\"headerRow\">" . lang("itemsinv_txt_7", true) . "</th>\r\n                <th class=\"headerRow\">" . lang("itemsinv_txt_8", true) . "</th>\r\n                <th class=\"headerRow\">" . lang("itemsinv_txt_9", true) . "</th>\r\n            </tr>";
            if (!isset($_GET["pg"])) {
                $_GET["pg"] = 1;
            }
            $myItems = $Webshop->getItemsInventory($_SESSION["username"], $_GET["pg"], mconfig("page_limit"));
            $i = 1;
            if (is_array($myItems)) {
                foreach ($myItems as $thisItem) {
                    $itemInfo = $Items->ItemInfo($thisItem["item"], $_SESSION["username"], NULL, 1);
                    switch ($thisItem["price_type"]) {
                        case "-1":
                            $price = number_format($thisItem["price"]) . " " . lang("exchange_txt_7", true);
                            break;
                        case "0":
                            $price = "--";
                            break;
                        case "1":
                            $price = number_format($thisItem["price"]) . " " . lang("currency_platinum", true);
                            break;
                        case "2":
                            $price = number_format($thisItem["price"]) . " " . lang("currency_gold", true);
                            break;
                        case "4":
                            $price = number_format($thisItem["price"]) . " " . lang("currency_silver", true);
                            break;
                        case "8":
                            $price = number_format($thisItem["price"]) . " " . lang("currency_wcoinc", true);
                            break;
                        case "9":
                            $price = number_format($thisItem["price"]) . " " . lang("currency_gp", true);
                            break;
                        case "10":
                            $price = number_format($thisItem["price"]) . " " . lang("currency_zen", true) . "";
                            break;
                        case "11":
                            $price = number_format($thisItem["price"]) . " " . lang("currency_bless", true) . "";
                            break;
                        case "12":
                            $price = number_format($thisItem["price"]) . " " . lang("currency_soul", true) . "";
                            break;
                        case "13":
                            $price = number_format($thisItem["price"]) . " " . lang("currency_life", true) . "";
                            break;
                        case "14":
                            $price = number_format($thisItem["price"]) . " " . lang("currency_chaos", true) . "";
                            break;
                        case "15":
                            $price = number_format($thisItem["price"]) . " " . lang("currency_harmony", true) . "";
                            break;
                        case "16":
                            $price = number_format($thisItem["price"]) . " " . lang("currency_creation", true) . "";
                            break;
                        case "17":
                            $price = number_format($thisItem["price"]) . " " . lang("currency_guardian", true) . "";
                            break;
                        default:
                            ...
                            echo "\r\n            <tr style=\"cursor: pointer;\">\r\n                <td align=\"center\">" . intval($_GET["pg"] * mconfig("page_limit") - mconfig("page_limit") + $i) . "</td>\r\n                <td align=\"left\" onmouseover=\"Tip(" . $Items->generateItemTooltip($itemInfo, 1, 1, 1, 1, 0, 0) . ")\" onmouseout=\"UnTip()\" <img src=\"" . $itemInfo["thumb"] . "\" class=\"m items-inventory-item-bg\">\r\n                    <span style=\"color:" . $itemInfo["color"] . ";background-color:" . $itemInfo["anco"] . "\">" . $itemInfo["name"] . "</span>\r\n                </td>\r\n                <td align=\"center\">" . $price . "</td>\r\n                <td align=\"center\">" . date($config["time_date_format"], strtotime($thisItem["date"])) . "</td>\r\n                <td align=\"center\">" . $type . "</td>\r\n                <td>\r\n                    <a href=\"" . __BASE_URL__ . "usercp/items/?action=withdraw&item=" . encode($thisItem["id"]) . "\">\r\n                        " . lang("itemsinv_txt_15", true) . "\r\n                    </a>\r\n                </td>\r\n            </tr>";
                            $i++;
                    }
                }
            } else {
            ...
            $total_items = $Webshop->getTotalItemsInventory($_SESSION["username"]);
            $total_pages = ceil($total_items / $limit);
            generatePagination($total_pages, $_GET["pg"], __BASE_URL__ . "usercp/items/pg/%pageHolder%/");
            echo "\r\n        </ul>\r\n    </nav>";
        } else {
